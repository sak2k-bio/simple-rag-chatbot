...
// Sample a small set of payload snippets from the collection for suggestions
export async function samplePayloadSnippets(sampleSize: number = 20, snippetChars: number = 300) {
    const client = getQdrantClient();
    const collection = process.env.QDRANT_COLLECTION;
    if (!collection) throw new Error('QDRANT_COLLECTION is not set');

    // Best-effort scroll: fetch a page and then sample from it
    // This is not perfectly random across the entire collection but sufficient for UX suggestions
    const limit = Math.max(sampleSize * 3, 30);
    const res: any = await (client as any).scroll(collection, {
        limit,
        with_payload: true,
        with_vectors: false,
    });

    const points: Array<{ payload: QdrantPointPayload }> = Array.isArray(res?.points) ? res.points : (Array.isArray(res) ? res : []);

    const items = points.map(p => {
        const payload = p.payload || {};
        const text = (payload as any).text || (payload as any).content || (payload as any).pageContent || '';
        const source = (payload as any).metadata?.source || (payload as any).source || 'Unknown';
        const clean = String(text).replace(/\s+/g, ' ').trim();
        return { source, text: clean.slice(0, snippetChars) };
    }).filter(i => i.text.length > 40);

    // Randomly pick up to sampleSize
